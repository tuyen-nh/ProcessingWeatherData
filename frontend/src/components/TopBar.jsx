...
export default function TopBar({ titleKey, subKey }) {
  const { t } = useTranslation()
  const { cityId, setCityId } = useCity()
  return (
    <header className="flex flex-wrap items-end justify-between gap-6 px-10 pt-10 pb-6 border-b border-line">
      <div>
        {subKey && (
          <div className="label mb-2">
            {t('top.station')} — {t(subKey)}
          </div>
        )}
        <h2 className="display text-4xl font-semibold tracking-tight text-ink">{t(titleKey)}</h2>
      </div>

      <div className="flex items-end gap-5">
        <label className="flex flex-col items-end gap-1.5">
          <span className="label">{t('top.activeCity')}</span>
